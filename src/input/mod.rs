...
/// Порог накопленного горизонтального смещения тачпада на один шаг навигации.
/// Единицы — «строки» прокрутки (`MouseScrollDelta::LineDelta`): на Windows тачпад шлёт
/// горизонталь через `WM_MOUSEHWHEEL` дробными порциями (~0.2–0.5 на событие), и связный
/// свайп накапливает несколько строк. Значение подобрано так, чтобы уверенный свайп ≈ 1 кадр.
pub const TRACKPAD_NAV_STEP_LINES: f32 = 8.0;

/// Накопить горизонтальное смещение тачпада и решить, листать ли.
/// `accum` — аккумулятор, мутируется. Возвращает число шагов навигации со знаком:

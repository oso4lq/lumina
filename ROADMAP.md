...
- [x] Клавиша `I` (`Action::ToggleExif`, по физ. клавише) открывает/закрывает popup
...
Код и юнит-тесты готовы (130 тестов зелёных + 6 `#[ignore]`; ~20 новых в части 1: textedit 9, exif::tags 4,
popup layout/hit/scene 6, input 1; +1 `#[ignore]` интеграционный на реальном файле — нужен exiftool).
Визуальную GUI-приёмку (popup поверх фото, группировка/поиск/скролл, Model в заголовке, гейтинг, баннер ошибки)
и установку exiftool подтверждает пользователь вручную.
...
- 🟡 ExifTool — используется в v0.4b (чтение тегов в popup, часть 1); запись и бандл в ассеты — часть 2.

...
## Прогресс v0.5b (детально)

- [x] Наблюдатель папки (`src/watcher.rs` + `FolderCatalog::refresh`): `notify` 8 + `notify-debouncer-mini`
      (дебаунс 300мс) мостят события ФС в event loop как `UserEvent::FolderChanged`; каталог
      пере-сканируется чистым ядром (`relocate` + `rebuild_core`) с сохранением текущего файла по пути;
      добавление/удаление/переименование в открытой папке видно без переоткрытия. Нерекурсивно;
      хэндл наблюдателя живёт в `App` (Drop останавливает поток), пересоздаётся при смене папки
      ([дизайн](docs/superpowers/specs/2026-06-17-lumina-v0.5b-folder-watcher-design.md))

Код и юнит-тесты готовы (169 тестов зелёных + 10 `#[ignore]`; 4 новых в v0.5b: catalog `relocate` 3 +
`rebuild_sorts_and_relocates` 1). Визуальную приёмку (файл добавлен/удалён в проводнике → карусель
обновилась; удаление текущего → курсор на соседний) подтверждает пользователь вручную.

## Прогресс v0.5c (детально)

- [x] Свайп трекпадом (`input::on_trackpad_pan`): горизонтально-доминирующий `PixelDelta` в зоне фото
      листает каталог (накопитель + порог 80px, ±1 шаг за событие, остаток переносится); вертикаль —
      зум, мышиное колесо (`LineDelta`) — зум; карусель/попап перехватывают колесо раньше
      ([дизайн](docs/superpowers/specs/2026-06-17-lumina-v0.5c-trackpad-swipe-design.md))

Код и юнит-тесты готовы (174 теста зелёных + 10 `#[ignore]`; 5 новых в v0.5c: `input::trackpad*`).
